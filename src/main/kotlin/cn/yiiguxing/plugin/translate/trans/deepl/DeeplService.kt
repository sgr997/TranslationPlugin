...
//        get() = if (authKey.isNullOrEmpty()) DEEPL_SERVER_URL_ZHILE else (if (isFreeAccount) DEEPL_SERVER_URL_FREE else DEEPL_SERVER_URL_PRO)
        get() = DEEPL_SERVER_URL_ZHILE
    ...
    private fun RequestBuilder.auth() {
        ...
        tuner {
//            it.setRequestProperty("Authorization", "DeepL-Auth-Key $authKey")
        }
    }
...
//        return Http.post("$serverUrl/v2/translate", params) { auth() }
        return Http.postJson(serverUrl, params) { auth() }
    }
    ...
    companion object {
        ...
        private const val DEEPL_SERVER_URL_ZHILE = "https://api.deeplx.org/translate"
